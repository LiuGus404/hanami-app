  ...
  useEffect(() => {
    ...
    const loadPricing = async () => {
      try {
        ...
        const activeTypes = (courseTypesAll || []).filter(ct => ct.status !== false);
        // 嘗試用關鍵字挑選兩個目標課程，若找不到則用 is_featured 或全部 active
        const keywords = ['幼兒', '專注', 'Piano', '鋼琴'];
        let targetTypes = activeTypes.filter(ct => keywords.some(k => (ct.name || '').includes(k)));
        if (targetTypes.length === 0) {
          targetTypes = activeTypes; // 後備：全部 active
        }
        const typeIdList = activeTypes.map(ct => ct.id);
        const typeMap: Record<string, any> = {};
        activeTypes.forEach(ct => { typeMap[ct.id] = ct; });
        ...
        let { data: plans, error: planErr } = await supabase
          ...
          .in('course_type_id', targetTypes.map(t => t.id))
          .eq('is_active', true)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-[#4B4036]">幼兒鋼琴啟蒙班</h1>
                <p className="text-sm text-[#2B3A3B]">Piano Class</p>
              </div>
            </div>
      ...
      <div className="flex-1 flex">
        ...
        {user && (
          <AppSidebar 
            ...
            currentPath="/aihome/hanami-music/piano-class"
          />
        )}
        ...
        <div className="flex-1 flex flex-col">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
            ...
            >
              <div className="bg-white rounded-3xl p-8 shadow-xl border border-[#EADBC8]/50">
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-bold text-[#4B4036] mb-4">不一樣的鋼琴教學法</h2>
                  <p className="text-lg text-[#2B3A3B]">以最有趣活潑又科學的音樂教學助孩子成長發展</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-bold text-[#4B4036] mb-2">主題繪本</h3>
                    <p className="text-[#2B3A3B]">透過故事引導音樂學習</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-bold text-[#4B4036] mb-2">獨創遊戲教具</h3>
                    <p className="text-[#2B3A3B]">讓學習變得有趣而有效</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-bold text-[#4B4036] mb-2">成長發展遊戲</h3>
                  ...
                  <div className="text-center">
                    <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-[#FFB6C1] to-[#FFD59A] rounded-2xl flex items-center justify-center shadow-lg overflow-hidden">
                      <img 
                        src="/HanamiMusic/MusicTheory.png" 
                        alt="樂理遊戲" 
                        className="w-full h-full object-cover"
                      />
                    </div>
                    <h3 className="text-xl font-bold text-[#4B4036] mb-2">樂理遊戲</h3>
                    <p className="text-[#2B3A3B]">輕鬆學習音樂理論</p>
                  </div>
                ...
                <div className="mt-8 p-6 bg-gradient-to-r from-[#FFD59A]/10 to-[#EBC9A4]/10 rounded-2xl border border-[#FFD59A]/20">
                  <p className="text-center text-lg text-[#4B4036] font-semibold">
                    配合 <span className="text-[#E74C3C]">科學化鋼琴訓練</span>，加強幼兒發展需要，繼而提升鋼琴演奏能力
                  </p>
                </div>
              </div>
            </motion.section>
            ...
            >
              <div className="bg-white rounded-3xl p-8 shadow-xl border border-[#EADBC8]/50">
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-bold text-[#4B4036] mb-4">多份研究證明 學琴有助</h2>
                  <p className="text-lg text-[#2B3A3B]">科學化音樂教學法，為2.5歲以上度身訂造</p>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {pricingPlans
                      .filter((plan) => {
                        ...
                        return name.includes('鋼琴') || name.includes('piano');
                      })
                      .map((plan, index) => {
                          ...
                          {plan.is_featured && (
                            <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                              <div className={`px-4 py-1 rounded-full text-xs font-bold text-white ${
                                plan.plan_type === 'trial' ? 'bg-red-500' : plan.plan_type === 'premium' ? 'bg-purple-500' : 'bg-blue-500'
                              }`}>
