// app/aihome/parent/student/[id]/page.tsx
'use client';
...
export default function ParentStudentDetailPage() {
  ...
  // 處理返回按鈕
  const handleBack = () => {
    router.push('/aihome/parent/connect');
  };
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (inactiveData) {
          // 由於 institution 欄位可能不存在，暫時跳過機構檢查
        ...
        // 檢查機構是否匹配
        if (studentData.institution !== institution) {
          setError(`此學生不屬於 ${institution} 機構，請選擇正確的機構或聯繫機構獲取正確的學生 ID`);
          setPageLoading(false);
          loadingRef.current = false;
          return;
        }
