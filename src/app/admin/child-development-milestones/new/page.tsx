...
export default function NewAgeGroupPage() {
  ...
  const [formData, setFormData] = useState<AgeGroupForm>({
    age_months: 0,
    age_description: '',
    age_range_min: 0,
    age_range_max: 0,
  ...
  const handleSave = async () => {
    // 驗證必要欄位
    if (!formData.age_description.trim()) {
      toast.error('請輸入年齡描述');
      return;
    }
    if (!formData.age_months || formData.age_months <= 0) {
      toast.error('請輸入有效的月齡');
      return;
    }
    ...
    try {
      ...
      // 由於 hanami_child_development_milestones 表可能不存在，使用 any 類型來繞過類型檢查
      ...
      toast.success('年齡組創建成功');
      router.push('/admin/child-development-milestones');
    } catch (error) {
      console.error('創建年齡組失敗:', error);
      toast.error('創建年齡組失敗');
    } finally {
      setSaving(false);
    }
    ...
    <div className="min-h-screen bg-[#FFF9F2] font-['Quicksand',_sans-serif] px-6 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#2B3A3B] mb-2">新增年齡組</h1>
          <p className="text-[#4B4036]">創建新的兒童發展里程碑年齡組</p>
        </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {developmentFields.map((field) => (
                  <div key={field.key} className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <textarea
                      value={formData[field.key as keyof AgeGroupForm] || ''}
