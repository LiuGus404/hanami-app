...
export async function GET(request: NextRequest) {
  try {
    ...
    // 從新 Supabase 獲取用戶會話
    console.log('[API /api/members/identities/list] 開始獲取會話...');
    console.log('[API /api/members/identities/list] 請求 URL:', request.url);
    console.log('[API /api/members/identities/list] 請求 headers:', {
    ...
    console.log('[API /api/members/identities/list] 會話結果:', {
      hasSession: !!session,
    ...
    if (!session?.user?.email) {
      console.log('[API /api/members/identities/list] 嘗試備選認證方式...');
      
      // 方法 1: 從 Authorization header 獲取（如果前端傳遞了用戶 email）
      const authHeader = request.headers.get('X-User-Email');
      if (authHeader) {
        console.log('[API /api/members/identities/list] 從 X-User-Email header 獲取 email:', authHeader);
        session = {
          user: {
            email: authHeader,
            id: '', // 暫時為空，後續可以從資料庫查詢
        ...
        if (emailParam) {
          console.log('[API /api/members/identities/list] 從查詢參數獲取 email:', emailParam);
          session = {
            user: {
              email: emailParam,
              id: '',
    ...
    if (!session?.user?.email) {
      console.error('[API /api/members/identities/list] ❌ 未授權：無法獲取用戶會話');
      // 調試：列出所有 cookies
      const { cookies } = await import('next/headers');
      const cookieStore = await cookies();
      const allCookies = cookieStore.getAll();
      console.log('[API /api/members/identities/list] 所有 cookies:', allCookies.map(c => ({
        name: c.name,
        hasValue: !!c.value,
    ...
    console.log('[API /api/members/identities/list] 身份查詢結果:', {
    ...
    console.log('[API /api/members/identities/list] 管理員查詢結果:', {
      hasAdmin: !!admin,
    ...
    console.log('[API /api/members/identities/list] 權限檢查結果:', {
      isAuthorized,
      hasIdentity: !!identity,
      hasAdmin: !!admin,
    });
    ...
    if (!isAuthorized) {
      console.warn('[API /api/members/identities/list] ❌ 無權限管理該機構');
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : '獲取成員列表時發生錯誤' },
      { status: 500 }
    );
  }
}
