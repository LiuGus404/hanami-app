...
export async function POST(request: NextRequest) {
  try {
    ...
    const { studentIds, orgId, userEmail } = body;
    ...
    if (deleteError) {
      console.error('批量刪除學生失敗:', deleteError);
      return NextResponse.json(
        { error: deleteError.message || '批量刪除學生失敗' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `成功刪除 ${studentIds.length} 個學生`,
      count: studentIds.length
    });

  } catch (error: any) {
    console.error('批量刪除學生失敗:', error);
    return NextResponse.json(
      { error: error?.message || '批量刪除學生失敗' },
      { status: 500 }
    );
  }
}
