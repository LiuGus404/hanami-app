...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bindings && bindings.length > 0) {
      ...
      // 查詢試堂學生
      const { data: specificTrialStudents, error: specificTrialError } = await adminClient
        .from('hanami_trial_students')
        .select('*')
        .in('id', studentIds);

      // 查詢常規學生
      const { data: specificRegularStudents, error: specificRegularError } = await adminClient
        .from('Hanami_Students')
        .select('*')
        .in('id', studentIds);
