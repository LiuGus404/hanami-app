...
interface TeachingActivityDetailModalProps {
  activity: any;
  onClose: () => void;
  onEdit?: () => void;
  onDelete?: () => void;
...
export default function TeachingActivityDetailModal({ 
  activity, 
  onClose, 
  onEdit, 
  onDelete, 
}: TeachingActivityDetailModalProps) {
          ...
          <HanamiCard className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{activity.estimated_duration || activity.duration_minutes || 0} 分鐘</span>
              </div>
              
              <div className="flex items-center gap-2">
                <StarIcon className="w-5 h-5 text-gray-400" />
                <span className="text-sm text-gray-600">難度：</span>
                <span className="font-medium">等級 {activity.difficulty_level || '未設定'}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">
                  {activity.created_at ? new Date(activity.created_at).toLocaleDateString() : '未設定'}
                </span>
              </div>
            </div>
          </HanamiCard>
