...
export default function FinalVideoTest({ 
  onScanSuccess, 
  onScanError, 
  onClose, 
  isActive 
  ...
  const addDebugInfo = (info: string) => {
    console.log(`[FinalVideoTest] ${info}`);
    setDebugInfo(prev => [...prev, `${new Date().toLocaleTimeString()}: ${info}`]);
  };

  // 組件加載時立即顯示調試信息
  useEffect(() => {
    addDebugInfo('FinalVideoTest 組件已加載');
    if (isActive) {
  ...
  useEffect(() => {
    if (isActive) {
      addDebugInfo('等待視頻元素準備...');
      
      // 使用 setTimeout 確保 DOM 已渲染
      const checkVideoElement = () => {
        if (videoRef.current) {
          addDebugInfo('視頻元素已準備好');
          setVideoReady(true);
        } else {
          addDebugInfo('視頻元素仍未準備好，繼續等待...');
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-[#4B4036]">掃描 QR 碼 (最終版)</h3>
          <button
            onClick={handleClose}
