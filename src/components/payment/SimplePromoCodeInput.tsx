...
import { 
  validateSimplePromoCode, 
  formatSimpleDiscountDisplay, 
  formatCurrency, 
  isValidPromoCodeFormat, 
  cleanPromoCode 
} from '@/lib/simplePromoCodeUtils';
...
export default function SimplePromoCodeInput({
  originalAmount,
  currency = 'HKD',
  userId,
  userEmail,
  ...
  const handleApplyPromoCode = async () => {
    ...
    try {
      const result = await validateSimplePromoCode(
        cleanedCode,
        originalAmount,
        userId,
        userEmail,
      ...
      if (result.success && result.data) {
        if (result.data.is_valid) {
          const discount: SimpleDiscountInfo = {
            code: cleanedCode,
            name: '', // 名稱會在後續 API 中獲取
          ...
          <div className="flex items-center justify-between p-3 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                {discountInfo.institution_name && (
                  <p className="text-xs text-green-600">{discountInfo.institution_name}</p>
                )}
                <p className="text-sm text-green-700">
                  {formatSimpleDiscountDisplay(discountInfo)} - 節省 {formatCurrency(discountInfo.discount_amount, currency)}
                </p>
              </div>
            </div>
            <button
