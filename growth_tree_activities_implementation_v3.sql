-- 成長樹活動欄位實作腳本 (基於現有表結構修正版)
-- 版本: 3.0.0
-- 日期: 2024-12-19
-- 描述: 基於現有 hanami_tree_activities 關聯表的實作方案

-- ========================================
-- 1. 分析現有表結構
-- ========================================

-- 現有 hanami_tree_activities 表結構：
-- id uuid not null default gen_random_uuid(),
-- tree_id uuid null,
-- activity_id uuid null,  -- 關聯到 hanami_teaching_activities
-- goal_id uuid null,      -- 關聯到 hanami_growth_goals
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'hanami_tree_activities') THEN
    ...
    -- 添加活動類型欄位（用於區分活動來源）
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hanami_tree_activities' AND column_name = 'activity_source') THEN
      ALTER TABLE hanami_tree_activities ADD COLUMN activity_source TEXT DEFAULT 'teaching' CHECK (activity_source IN ('teaching', 'custom', 'template'));
    END IF;
    
    -- 添加自訂活動名稱欄位（當 activity_source = 'custom' 時使用）
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hanami_tree_activities' AND column_name = 'custom_activity_name') THEN
      ALTER TABLE hanami_tree_activities ADD COLUMN custom_activity_name TEXT;
    END IF;
    
    -- 添加自訂活動描述欄位
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hanami_tree_activities' AND column_name = 'custom_activity_description') THEN
      ALTER TABLE hanami_tree_activities ADD COLUMN custom_activity_description TEXT;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hanami_tree_activities' AND column_name = 'activity_type') THEN
      ALTER TABLE hanami_tree_activities ADD COLUMN activity_type TEXT DEFAULT 'custom' CHECK (activity_type IN ('custom', 'teaching', 'assessment', 'practice', 'game'));
    END IF;
    ...
    -- 添加是否活躍欄位
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'hanami_tree_activities' AND column_name = 'is_active') THEN
      ALTER TABLE hanami_tree_activities ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_tree_activities_activity_id ON hanami_tree_activities(activity_id);
CREATE INDEX IF NOT EXISTS idx_tree_activities_goal_id ON hanami_tree_activities(goal_id);
CREATE INDEX IF NOT EXISTS idx_tree_activities_activity_source ON hanami_tree_activities(activity_source);
...
CREATE INDEX IF NOT EXISTS idx_tree_activities_priority_order ON hanami_tree_activities(priority_order);
...
CREATE INDEX IF NOT EXISTS idx_tree_activities_tree_source ON hanami_tree_activities(tree_id, activity_source);
...
-- 獲取成長樹的所有活動（包含關聯的教學活動資訊）
CREATE OR REPLACE FUNCTION get_tree_activities_with_details(p_tree_id UUID)
RETURNS TABLE (
  id UUID,
  tree_id UUID,
  activity_id UUID,
  ...
  -- 關聯的教學活動資訊
  teaching_activity_name TEXT,
  teaching_activity_description TEXT,
  teaching_activity_type TEXT,
  teaching_difficulty_level INTEGER,
  teaching_duration_minutes INTEGER,
  teaching_materials_needed TEXT[],
...
BEGIN
  ...
  SELECT 
    ...
    ta.goal_id,
    ta.activity_source,
    ta.custom_activity_name,
    ta.custom_activity_description,
    ta.activity_type,
    ta.difficulty_level,
    ...
    ta.created_at,
    -- 關聯的教學活動資訊
    tea.activity_name as teaching_activity_name,
    tea.activity_description as teaching_activity_description,
    tea.activity_type as teaching_activity_type,
    tea.difficulty_level as teaching_difficulty_level,
    tea.duration_minutes as teaching_duration_minutes,
    tea.materials_needed as teaching_materials_needed,
    tea.instructions as teaching_instructions
  FROM hanami_tree_activities ta
  LEFT JOIN hanami_teaching_activities tea ON ta.activity_id = tea.id
  WHERE ta.tree_id = p_tree_id
    AND ta.is_active = true
  ORDER BY COALESCE(ta.activity_order, ta.priority_order), ta.created_at;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  RETURN QUERY
  SELECT 
    ta.id as tree_activity_id,
    ta.activity_source,
    CASE 
      WHEN ta.activity_source = 'custom' THEN ta.custom_activity_name
      WHEN ta.activity_source = 'teaching' THEN tea.activity_name
      ELSE ta.custom_activity_name
    END as activity_name,
    COALESCE(ta.activity_type, tea.activity_type) as activity_type,
    COALESCE(ta.difficulty_level, tea.difficulty_level) as difficulty_level,
...
BEGIN
  ...
  WHERE student_id = p_student_id AND tree_activity_id = p_tree_activity_id;
...
COMMENT ON TABLE hanami_tree_activities IS '成長樹活動關聯表（擴展版）';
COMMENT ON COLUMN hanami_tree_activities.activity_source IS '活動來源：teaching=教學活動，custom=自訂活動，template=模板活動';
COMMENT ON COLUMN hanami_tree_activities.custom_activity_name IS '自訂活動名稱（當 activity_source = custom 時使用）';
...
COMMENT ON COLUMN hanami_tree_activities.priority_order IS '優先順序（現有欄位）';
COMMENT ON COLUMN hanami_tree_activities.activity_order IS '活動順序（新增欄位）';
...
SELECT '已擴展以下表：' as info;
SELECT '  - hanami_tree_activities (成長樹活動關聯表 - 擴展版)' as table_name;
...
SELECT '支援三種活動來源：教學活動、自訂活動、模板活動' as feature; 