...
CREATE TABLE public.ai_messages (
  ...
  status text DEFAULT 'sent'::text CHECK (status = ANY (ARRAY['queued'::text, 'processing'::text, 'sent'::text, 'error'::text, 'cancelled'::text, 'deleted'::text])),
  error_message text,
  ...
  CONSTRAINT ai_messages_sender_role_instance_id_fkey FOREIGN KEY (sender_role_instance_id) REFERENCES public.role_instances(id)
);
CREATE TABLE public.ai_roles (
  ...
  updated_at timestamp with time zone DEFAULT now(),
  tone text,
  available_models ARRAY DEFAULT ARRAY['gpt-4o-mini'::text],
  available_model_ids ARRAY DEFAULT '{}'::uuid[],
...
CREATE TABLE public.chat_messages (
  ...
  status text NOT NULL DEFAULT 'queued'::text CHECK (status = ANY (ARRAY['queued'::text, 'processing'::text, 'completed'::text, 'error'::text, 'cancelled'::text, 'sending'::text, 'deleted'::text])),
...
CREATE TABLE public.content_library (
  ...
  api_owner_username text,
  api_owner_fullname text,
  api_caption text,
  api_timestamp timestamp with time zone,
  api_likes integer,
  api_comments integer,
  api_video_views integer,
  api_video_plays integer,
  api_hashtags jsonb,
  api_raw jsonb,
  metrics_extra jsonb,
  main_text text,
  ...
  section_hook text,
  section_cta text,
  section_script_outline text,
  section_subtitles_raw text,
  subtitle_lines jsonb,
  subtitle_lines_count integer,
  analysis_model text,
  analysis_version integer,
  analysis_created_at timestamp with time zone,
  status text DEFAULT 'raw'::text,
  my_rating smallint,
  my_priority smallint,
  my_notes text,
  my_idea_links jsonb,
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT content_library_pkey PRIMARY KEY (id),
  CONSTRAINT content_library_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.food_transactions (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  transaction_type text NOT NULL CHECK (transaction_type = ANY (ARRAY['deposit'::text, 'withdrawal'::text, 'usage'::text, 'initial_grant'::text, 'purchase'::text, 'other'::text, 'refund'::text, 'adjustment'::text])) NOT VALI),
  ...
  CONSTRAINT food_transactions_ai_message_id_fkey FOREIGN KEY (ai_message_id) REFERENCES public.ai_messages(id),
  CONSTRAINT food_transactions_ai_room_id_fkey FOREIGN KEY (ai_room_id) REFERENCES public.ai_rooms(id)
);
...
CREATE TABLE public.hanami_organizations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT hanami_organizations_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.hanami_user_organizations (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_email text NOT NULL,
  user_id uuid,
  org_id uuid NOT NULL,
  role text DEFAULT 'member'::text CHECK (role = ANY (ARRAY['owner'::text, 'admin'::text, 'teacher'::text, 'member'::text])),
  is_primary boolean DEFAULT false,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT hanami_user_organizations_pkey PRIMARY KEY (id),
  CONSTRAINT hanami_user_organizations_org_id_fkey FOREIGN KEY (org_id) REFERENCES public.hanami_organizations(id)
);
...
CREATE TABLE public.message_costs (
  ...
  CONSTRAINT message_costs_ai_message_id_fkey FOREIGN KEY (ai_message_id) REFERENCES public.ai_messages(id),
  CONSTRAINT message_costs_ai_room_id_fkey FOREIGN KEY (ai_room_id) REFERENCES public.ai_rooms(id)
);
CREATE TABLE public.message_processing_logs (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  message_id uuid NOT NULL,
  workflow_name text NOT NULL,
  worker_id text,
  event_type text NOT NULL CHECK (event_type = ANY (ARRAY['queued'::text, 'processing'::text, 'completed'::text, 'error'::text, 'cancelled'::text, 'status_change'::text])),
  event_data jsonb DEFAULT '{}'::jsonb,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT message_processing_logs_pkey PRIMARY KEY (id),
  CONSTRAINT message_processing_logs_message_id_fkey FOREIGN KEY (message_id) REFERENCES public.chat_messages(id)
);
CREATE TABLE public.mind_block_likes (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  mind_block_id uuid NOT NULL,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT mind_block_likes_pkey PRIMARY KEY (id),
  CONSTRAINT mind_block_likes_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT mind_block_likes_mind_block_id_fkey FOREIGN KEY (mind_block_id) REFERENCES public.mind_blocks(id)
);
CREATE TABLE public.mind_block_reviews (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  mind_block_id uuid NOT NULL,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT mind_block_reviews_pkey PRIMARY KEY (id),
  CONSTRAINT mind_block_reviews_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT mind_block_reviews_mind_block_id_fkey FOREIGN KEY (mind_block_id) REFERENCES public.mind_blocks(id)
);
CREATE TABLE public.mind_blocks (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  title text NOT NULL,
  description text,
  icon text DEFAULT '🧩'::text,
  color text DEFAULT '#FFD59A'::text,
  ...
  CONSTRAINT mind_blocks_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT mind_blocks_forked_from_id_fkey FOREIGN KEY (forked_from_id) REFERENCES public.mind_blocks(id)
);
CREATE TABLE public.model_configs (
  ...
  context_window_display text,
  pricing_details jsonb DEFAULT '{}'::jsonb,
  supported_modalities ARRAY DEFAULT ARRAY['text'::text],
...
CREATE TABLE public.model_pricing (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT model_pricing_pkey PRIMARY KEY (provider, model)
);
CREATE TABLE public.model_pricing_history (
  ...
  change_reason text,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT model_pricing_history_pkey PRIMARY KEY (id),
  CONSTRAINT model_pricing_history_model_config_id_fkey FOREIGN KEY (model_config_id) REFERENCES public.model_configs(id)
);
...
CREATE TABLE public.role_mind_blocks (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  CONSTRAINT role_mind_blocks_role_id_fkey FOREIGN KEY (role_id) REFERENCES public.ai_roles(id),
  CONSTRAINT role_mind_blocks_mind_block_id_fkey FOREIGN KEY (mind_block_id) REFERENCES public.mind_blocks(id),
  CONSTRAINT role_mind_blocks_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.saas_gacha_draw_history (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  machine_id uuid NOT NULL,
  draw_type text NOT NULL CHECK (draw_type = ANY (ARRAY['single'::text, 'ten'::text])),
  points_spent integer NOT NULL,
  rewards_won jsonb NOT NULL,
  draw_count integer NOT NULL DEFAULT 1,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_gacha_draw_history_pkey PRIMARY KEY (id),
  CONSTRAINT saas_gacha_draw_history_machine_id_fkey FOREIGN KEY (machine_id) REFERENCES public.saas_gacha_machines(id),
  CONSTRAINT saas_gacha_draw_history_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
CREATE TABLE public.saas_gacha_machine_rewards (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  machine_id uuid NOT NULL,
  reward_id uuid NOT NULL,
  probability numeric NOT NULL CHECK (probability >= 0::numeric AND probability <= 100::numeric),
  weight integer NOT NULL DEFAULT 100,
  is_active boolean DEFAULT true,
  display_order integer DEFAULT 0,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_gacha_machine_rewards_pkey PRIMARY KEY (id),
  CONSTRAINT saas_gacha_machine_rewards_machine_id_fkey FOREIGN KEY (machine_id) REFERENCES public.saas_gacha_machines(id),
  CONSTRAINT saas_gacha_machine_rewards_reward_id_fkey FOREIGN KEY (reward_id) REFERENCES public.saas_gacha_rewards(id)
);
CREATE TABLE public.saas_gacha_machines (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  machine_name text NOT NULL,
  machine_slug text NOT NULL UNIQUE,
  description text,
  image_url text,
  background_url text,
  theme_config jsonb DEFAULT '{}'::jsonb,
  single_draw_cost integer NOT NULL DEFAULT 10,
  ten_draw_cost integer NOT NULL DEFAULT 90,
  ten_draw_bonus integer DEFAULT 1,
  is_active boolean DEFAULT true,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_gacha_machines_pkey PRIMARY KEY (id)
);
CREATE TABLE public.saas_gacha_rewards (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  reward_name text NOT NULL,
  reward_description text,
  reward_type text NOT NULL CHECK (reward_type = ANY (ARRAY['discount_coupon'::text, 'free_trial'::text, 'course_voucher'::text, 'vip_upgrade'::text, 'subscription_extend'::text, 'physical_gift'::text, 'points_bonus'::text, 'custom'::text])),
  ...
  usage_limit integer,
  valid_days integer,
  delivery_type text NOT NULL DEFAULT 'auto'::text CHECK (delivery_type = ANY (ARRAY['auto'::text, 'manual'::text, 'physical'::text])),
  stock_total integer,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_gacha_rewards_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.saas_point_earning_rules (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  rule_name text NOT NULL,
  rule_type text NOT NULL CHECK (rule_type = ANY (ARRAY['daily_login'::text, 'task_completion'::text, 'referral'::text, 'purchase_ratio'::text, 'event'::text])),
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_point_earning_rules_pkey PRIMARY KEY (id)
);
CREATE TABLE public.saas_point_transactions (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  transaction_type text NOT NULL CHECK (transaction_type = ANY (ARRAY['earn_login'::text, 'earn_task'::text, 'earn_referral'::text, 'earn_purchase'::text, 'earn_event'::text, 'earn_admin'::text, 'spend_gacha'::text, 'spend_redeem'::text, 'expire'::text, 'refund'::text])),
  ...
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_point_transactions_pkey PRIMARY KEY (id),
  CONSTRAINT saas_point_transactions_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.saas_user_points (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_user_points_pkey PRIMARY KEY (id),
  CONSTRAINT saas_user_points_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
CREATE TABLE public.saas_user_rewards (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  reward_id uuid NOT NULL,
  draw_history_id uuid,
  reward_code text UNIQUE,
  status text NOT NULL DEFAULT 'active'::text CHECK (status = ANY (ARRAY['active'::text, 'used'::text, 'expired'::text, 'cancelled'::text])),
  usage_count integer DEFAULT 0,
  usage_limit integer,
  obtained_at timestamp with time zone DEFAULT now(),
  expires_at timestamp with time zone,
  used_at timestamp with time zone,
  delivery_status text CHECK (delivery_status = ANY (ARRAY['pending'::text, 'processing'::text, 'shipped'::text, 'delivered'::text, 'cancelled'::text])),
  delivery_address jsonb,
  tracking_number text,
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT saas_user_rewards_pkey PRIMARY KEY (id),
  CONSTRAINT saas_user_rewards_reward_id_fkey FOREIGN KEY (reward_id) REFERENCES public.saas_gacha_rewards(id),
  CONSTRAINT saas_user_rewards_draw_history_id_fkey FOREIGN KEY (draw_history_id) REFERENCES public.saas_gacha_draw_history(id),
  CONSTRAINT saas_user_rewards_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
...
CREATE TABLE public.saas_users (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  user_role USER-DEFINED NOT NULL DEFAULT 'user'::saas_user_role CHECK (user_role = ANY (ARRAY['super_admin'::saas_user_role, 'admin'::saas_user_role, 'user'::saas_user_role, 'staff'::saas_user_role, 'moderator'::saas_user_role, 'billing_manager'::saas_user_role, 'auditor'::saas_user_role])),
...
CREATE TABLE public.user_role_settings (
  ...
  CONSTRAINT user_role_settings_role_id_fkey FOREIGN KEY (role_id) REFERENCES public.ai_roles(id),
  CONSTRAINT user_role_settings_model_config_id_fkey FOREIGN KEY (model_config_id) REFERENCES public.model_configs(id)
);